...
const QuestionsManage = ({ isEmbedded = false }) => {
    ...
    return (
        <div className={containerClass}>
            {!isEmbedded && (
                <Helmet>
                    <title>Manage Questions - Umutoza</title>
                </Helmet>
            )}
            ...
            {!isEmbedded && (
                <nav className="bg-white border-b border-slate-100 px-6 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm">
                    ...
                    <div className="flex items-center space-x-4">
                        <button onClick={() => navigate('/')} className="text-sm font-bold text-slate-400 hover:text-indigo-600 transition-colors">Home</button>
